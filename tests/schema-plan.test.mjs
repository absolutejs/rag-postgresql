...
describe("createPostgresSchemaPlan", () => {
  test("builds extension, schema, table, and hnsw index SQL", () => {
    ...
    expect(plan.extensionSql).toEqual([
      'CREATE EXTENSION IF NOT EXISTS "vector"',
    ]);
    expect(plan.schemaSql).toEqual([
      'CREATE SCHEMA IF NOT EXISTS "absolute_rag"',
    ]);
    ...
    expect(plan.migrationTableQualifiedName).toBe(
      '"absolute_rag"."migrations"',
    );
  });
...
describe("createPostgresMigrationPlan", () => {
  test("creates deterministic bootstrap and tracked migrations", () => {
    ...
    expect(plan.migrations.some((entry) => entry.stage === "extension")).toBe(
      true,
    );
    expect(
      plan.migrations.some((entry) =>
        entry.sql.includes('"absolute_rag"."migrations"'),
      ),
    ).toBe(false);
