...
describe("createPgvectorStore", () => {
  test("initializes schema and upserts/query/clear through the client boundary", async () => {
    ...
    mock.responders.push({
      when: (sql) =>
        sql.startsWith('SELECT name FROM "absolute_rag"."migrations"'),
    ...
    expect(
      mock.calls.some((entry) =>
        entry.sql.includes("CREATE EXTENSION IF NOT EXISTS"),
      ),
    ).toBe(true);
    expect(
      mock.calls.some((entry) =>
        entry.sql.includes('INSERT INTO "absolute_rag"."migrations"'),
      ),
    ).toBe(true);
    expect(mock.calls.some((entry) => entry.sql.includes("INSERT INTO"))).toBe(
      true,
    );
    expect(
      mock.calls.some((entry) => entry.sql.includes("ORDER BY distance ASC")),
    ).toBe(true);
    expect(mock.calls.some((entry) => entry.sql.includes("DELETE FROM"))).toBe(
      true,
    );
  });
  ...
  test("applyPostgresMigrations reports pending vs skipped names", async () => {
    ...
    mock.responders.push({
      ...
      result: () => {
        ...
        return {
          rows: [
            { name: "01_extension_001_create_extension_if_not_exists_vector" },
          ],
          rowCount: 1,
        };
      },
    ...
    expect(dryRun.skippedNames).toContain(
      "01_extension_001_create_extension_if_not_exists_vector",
    );
